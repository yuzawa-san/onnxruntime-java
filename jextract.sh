...
docker run --rm -v `pwd`:/workdir onnxruntime-jextract --output ${GENERATED_DIR} -l onnxruntime --source --target-package com.jyuzawa.onnxruntime_extern -I /usr/include -I /usr/include/x86_64-linux-gnu -I ${HEADER_DIR} --dump-includes symbols.conf ${HEADER_FILE}
# strip out the irrelevant symbols
csplit symbols.conf "/headers/"
rm xx00
mv xx01 symbols.conf
docker run --rm -v `pwd`:/workdir onnxruntime-jextract --output ${GENERATED_DIR} -l onnxruntime --source --target-package com.jyuzawa.onnxruntime_extern -I /usr/include -I /usr/include/x86_64-linux-gnu -I ${HEADER_DIR} @symbols.conf ${HEADER_FILE}
