...
mkdir -p ${PACKAGE_DIR}
./jextract-19/bin/jextract --output ${GENERATED_DIR} -l onnxruntime --source --target-package com.jyuzawa.onnxruntime_extern -I /usr/include -I ${HEADER_DIR} --dump-includes ${SYMBOLS_CONF} onnxruntime_all.h
# strip out the irrelevant symbols
csplit ${SYMBOLS_CONF} "/onnxruntime_c_api.h/"
rm xx00
mv xx01 ${SYMBOLS_CONF}
./jextract-19/bin/jextract --output ${GENERATED_DIR} -l onnxruntime --source --target-package com.jyuzawa.onnxruntime_extern -I /usr/include -I ${HEADER_DIR} @${SYMBOLS_CONF} onnxruntime_all.h
