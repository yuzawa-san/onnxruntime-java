...
final class SessionImpl extends ManagedImpl implements Session {
  ...
  public SessionImpl(ApiImpl api, ResourceScope scope, MemoryAddress session) {
    ...
    MemoryAddress metadata =
        api.create(allocator, out -> api.SessionGetModelMetadata.apply(session, out));
  ...
  private interface GetName {
    MemoryAddress apply(
        MemoryAddress session, long idx, MemoryAddress ortAllocator, MemoryAddress out);
  }
  ...
  private static Map<String, NodeInfo> createMap(
    ...
    for (long i = 0; i < numInputs; i++) {
      final long j = i;
      MemoryAddress nameSegment =
          api.create(allocator, out -> getName.apply(session, j, ortAllocator, out));
      String name = toJavaString(nameSegment);
      api.checkStatus(api.AllocatorFree.apply(ortAllocator, nameSegment));
      MemoryAddress typeInfoAddress =
          api.create(allocator, out -> getTypeInfo.apply(session, j, out));
  ...
  public Output run(Transaction transaction) {
    ...
    try (ResourceScope scope = ResourceScope.newConfinedScope()) {
      ...
      MemoryAddress ioBinding =
          api.create(allocator, out -> api.CreateIoBinding.apply(address, out));
      ...
      for (Map.Entry<String, float[]> entry : transaction.getInputs().entrySet()) {
        ...
        MemoryAddress inputTensor =
            api.create(
                allocator,
                out ->
                    api.CreateTensorWithDataAsOrtValue.apply(
                        memoryInfo,
                        inputData.address(),
                        inputData.byteSize(),
                        shapeData.address(),
      ...
      for (String outputName : runOutputs) {
        ...
        MemoryAddress outputTensor =
            api.create(
                allocator,
                out ->
                    api.CreateTensorAsOrtValue.apply(
                        ortAllocator,
      ...
      for (Map.Entry<String, MemoryAddress> entry : outputValues.entrySet()) {
        ...
        long byteSize =
            tensorInfo.getType().getValueLayout().byteSize() * tensorInfo.getElementCount();
