...
final class ModelMetadataImpl implements ModelMetadata {
	...
	ModelMetadataImpl(ApiImpl api, MemoryAddress metadata, MemoryAddress ortAllocator){
		try(ResourceScope scope = ResourceScope.newConfinedScope()){
			...
			}
			{
				...
				api.checkStatus(api.ModelMetadataGetCustomMetadataMapKeys.apply(metadata, ortAllocator, keys.address(), count.address()));
				...
				for(long i = 0; i < numKeys; i++) {
					...
					MemoryAddress value = api.create( allocator, out-> api.ModelMetadataLookupCustomMetadataMap.apply(metadata, ortAllocator, key, out));
	...
	public String toString() {
		return "{ModelMetadata: graphName="+graphName+", version="+version+", producerName="+producerName+", domain="+domain+"}";
	}
